...
```zsh
make use-now
make use-now IDEA="The thing I keep circling is ..."
make work-block
make next
make now
make first-minute
...
These commands do not launch or foreground Terminal Brain. `make now` is the Use Now-first orientation for one move, the reason for that move, process truth, and readiness. `make processes` and MCP `terminal_brain_process_map_markdown` are the quickest answers for "what is still going" across Terminal Brain, Codex, MCP, kernel, Drafts, launchctl, and the localhost API.
