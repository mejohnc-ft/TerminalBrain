...
- Runtime Status, Doctor, and Next Move for humans and agents: `make next`, `make status`, `make doctor`, `terminal_brain_next_markdown`, and `terminal_brain_runtime_status` report what to do, repo, CI, process, launchctl, app install, MCP contract, agent config, and API state without requiring or launching the app.
...
- MCP tools for next move, runtime status, app status, snapshot, snapshot Markdown, setup, focus, Start Here, Value Brief, Oracle Digest, Agent Prompt, Blindspot Brief, Idea Pulse, Idea ask, Idea ask-and-commit, Blindspot ask, Blindspot ask-and-commit, Blindspot action, Operator Brief, focus ask, focus ask-and-commit, radar, sources, briefing, permissions, sync, Start Work, Oracle ask, ask-and-commit, outcome commit, idea capture, Oracle items, Oracle commits, and Oracle review status.
