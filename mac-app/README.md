...
- **First Minute**: Default landing surface with the shortest value explanation, next action, agent handoff, quick thought capture, context-pack path, and outcome proof loop.
...
- Command palette entries for asking the current Focus, copying the operator snapshot, First Minute, Operator Brief, Value Proof, Decision Lane, Project Memory, or Operator Deck, asking Oracle from typed text, drafting captured ideas, and building context packs.
...
- App Shortcuts for Ask Oracle, Capture Idea, Copy Handoff, Copy First Minute, Copy Brief, Copy Value Proof, Copy Decision Lane, Copy Project Memory, Copy Deck, Copy Snapshot, Run Sync, Start Work, Open Latest Context Pack, and Copy Latest Context Pack in Spotlight, Siri, and automation.
- Menu bar extra with current Focus, Ask Current Focus, Build Focus Pack, Open/Copy Latest Context Pack, Copy Agent Handoff, Copy First Minute, Copy Operator Brief, Copy Value Proof, Copy Decision Lane, Copy Project Memory, refresh, sync, copy, Mission, and logs.
...
The Oracle and snapshot helpers never launch or foreground Terminal Brain. Use `oracle.zsh` for a prompt-ready answer, `oracle.zsh --commit` to save it to the Oracle Inbox, `--brief-markdown` for the plain-language value read, `--today` for the ranked Decision Lane, `--projects` for Project Memory, `--deck` for app-aligned action-card JSON, `--deck-markdown` for paste-ready cards, `--latest-pack` for the newest context pack Markdown, and `--output` for handoff files.
