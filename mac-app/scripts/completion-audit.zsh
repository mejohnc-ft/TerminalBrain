...
echo "11. Proactive freshness and action-card surfaces turn stale source data into ranked next actions."
echo
...
evidence "Proactive freshness/action loop" "grep -q '^freshness:' '$ROOT/Makefile' && grep -q '^action-cards:' '$ROOT/Makefile' && grep -q '^daily-brief:' '$ROOT/Makefile' && grep -q 'terminal_brain_freshness_markdown' '$ROOT/mcp-server/expected-tools.json' && grep -q 'terminal_brain_action_cards_markdown' '$ROOT/mcp-server/expected-tools.json' && grep -q 'terminal_brain_daily_brief_markdown' '$ROOT/mcp-server/expected-tools.json'" "make freshness/action-cards/daily-brief and MCP equivalents expose stale sources and ranked action cards."
