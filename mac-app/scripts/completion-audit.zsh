...
evidence "Clean queue avoids churn" "grep -q 'make ask QUERY=.*cheap test' '$ROOT/mac-app/scripts/use-now.zsh' && grep -q 'Do not manufacture busywork' '$ROOT/mac-app/scripts/work-block.zsh'" "Use Now defaults to a direct read and Work Block suppresses fake work instead of creating repeated clean-queue notes."
