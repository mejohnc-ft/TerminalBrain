...
echo "| --- | --- | --- |"
evidence "One-command Use Now path" "grep -q 'Terminal Brain Use Now' '$ROOT/mac-app/scripts/use-now.zsh' && grep -q '## One Move' '$ROOT/mac-app/scripts/use-now.zsh' && grep -q '^use-now:' '$ROOT/Makefile' && grep -q '^start: use-now' '$ROOT/Makefile' && grep -q 'terminal_brain_use_now_markdown' '$ROOT/mcp-server/server.mjs'" "make start, make use-now, default one-move operator path, and MCP use-now surface"
