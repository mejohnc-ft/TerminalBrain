...
echo "| --- | --- | --- |"
evidence "One-command Use Now path" "grep -q 'Terminal Brain Use Now' '$ROOT/mac-app/scripts/use-now.zsh' && grep -q '## One Move' '$ROOT/mac-app/scripts/use-now.zsh' && grep -q '^use-now:' '$ROOT/Makefile' && grep -q 'terminal_brain_use_now_markdown' '$ROOT/mcp-server/server.mjs'" "make use-now, default one-move operator path, and MCP use-now surface"
...
evidence "Clean-queue operator menu" "grep -q 'Choose Your Mode' '$ROOT/mac-app/scripts/use-now.zsh' && grep -q 'Selected Signal' '$ROOT/mac-app/scripts/use-now.zsh' && grep -q 'make ask-commit QUERY=.*clean' '$ROOT/mac-app/scripts/use-now.zsh' && grep -q 'recent-work.zsh.*--dry-run' '$ROOT/mac-app/scripts/use-now.zsh' && grep -q '## Choose Your Mode' '$ROOT/mac-app/scripts/check-entrypoints.zsh'" "Use Now offers explicit pressure-test, recent-work, capture, delegate, and outcome lanes when no open item dominates"
