...
echo "6. The native app exposes the same high-value entry points through sections, copy actions, menus, and Shortcuts."
...
evidence "Native copy actions" "grep -q 'func copyDemo' '$ROOT/mac-app/Sources/TerminalBrain/BrainStatusModel.swift' && grep -q 'func copyPlaybook' '$ROOT/mac-app/Sources/TerminalBrain/BrainStatusModel.swift' && grep -q 'func copyValueAudit' '$ROOT/mac-app/Sources/TerminalBrain/BrainStatusModel.swift'" "in-app copy actions for high-value surfaces"
evidence "Native menus" "grep -q 'Copy Demo' '$ROOT/mac-app/Sources/TerminalBrain/TerminalBrainApp.swift' && grep -q 'Copy Playbook' '$ROOT/mac-app/Sources/TerminalBrain/TerminalBrainApp.swift' && grep -q 'Copy Value Audit' '$ROOT/mac-app/Sources/TerminalBrain/TerminalBrainApp.swift'" "menu and menu bar access"
evidence "Native shortcuts" "grep -q 'CopyDemoIntent' '$ROOT/mac-app/Sources/TerminalBrain/AppShortcuts.swift' && grep -q 'CopyPlaybookIntent' '$ROOT/mac-app/Sources/TerminalBrain/AppShortcuts.swift' && grep -q 'CopyValueAuditIntent' '$ROOT/mac-app/Sources/TerminalBrain/AppShortcuts.swift'" "App Shortcuts for high-value surfaces"
evidence "App API routes" "grep -q '\"/demo/markdown\"' '$ROOT/mac-app/Sources/TerminalBrain/LocalControlServer.swift' && grep -q '\"/playbook/markdown\"' '$ROOT/mac-app/Sources/TerminalBrain/LocalControlServer.swift' && grep -q '\"/value-audit/markdown\"' '$ROOT/mac-app/Sources/TerminalBrain/LocalControlServer.swift'" "app-backed Markdown endpoints for Demo, Playbook, and Value Audit"
