...
evidence "Clean-queue operator menu" "grep -q 'Choose Your Mode' '$ROOT/mac-app/scripts/use-now.zsh' && grep -q 'make ask-commit QUERY=.*clean' '$ROOT/mac-app/scripts/use-now.zsh' && grep -q 'recent-work.zsh.*--dry-run' '$ROOT/mac-app/scripts/use-now.zsh' && grep -q '## Choose Your Mode' '$ROOT/mac-app/scripts/check-entrypoints.zsh'" "Use Now offers explicit pressure-test, recent-work, capture, delegate, and outcome lanes when no open item dominates"
