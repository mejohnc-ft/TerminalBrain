...
echo "14. A clean queue gives a guided check-in instead of a raw placeholder command."
echo "15. The system proactively tracks source freshness and turns stale data, review items, memory leads, repo state, and UX blockers into ranked action cards."
echo
...
evidence "Clean-queue operator menu" "grep -q 'Choose Your Mode' '$ROOT/mac-app/scripts/use-now.zsh' && grep -q 'Selected Signal' '$ROOT/mac-app/scripts/use-now.zsh' && grep -q 'make answer' '$ROOT/mac-app/scripts/use-now.zsh' && grep -q '^answer:' '$ROOT/Makefile' && grep -q 'get one direct decision read' '$ROOT/mac-app/scripts/use-now.zsh' && grep -q 'recent-work.zsh.*--dry-run' '$ROOT/mac-app/scripts/use-now.zsh' && grep -q '## Choose Your Mode' '$ROOT/mac-app/scripts/check-entrypoints.zsh'" "Use Now offers a short direct answer first, then pressure-test, recent-work, capture, delegate, and outcome lanes when no open item dominates without creating automatic notes"
evidence "Guided clean-queue check-in" "grep -q 'Terminal Brain Check In' '$ROOT/mac-app/scripts/check-in.zsh' && grep -q 'Use It Immediately' '$ROOT/mac-app/scripts/check-in.zsh' && grep -q 'Given this check-in for' '$ROOT/mac-app/scripts/check-in.zsh' && grep -q '^check-in:' '$ROOT/Makefile' && grep -q 'terminal_brain_check_in_markdown' '$ROOT/mcp-server/server.mjs' && grep -q 'mcp_check_in_output' '$ROOT/mac-app/scripts/check-entrypoints.zsh' && grep -q 'make check-in' '$ROOT/mac-app/scripts/oracle.zsh' && grep -q 'Success looks like' '$ROOT/mac-app/scripts/oracle.zsh'" "make check-in and MCP Check In give a clean-queue operator three prompts, capture the answer as reviewable memory, ask against that exact thought, print an outcome close loop, and define what success looks like"
...
evidence "Proactive freshness and action cards" "grep -q '^freshness:' '$ROOT/Makefile' && grep -q '^action-cards:' '$ROOT/Makefile' && grep -q '^daily-brief:' '$ROOT/Makefile' && grep -q '^refresh-memory:' '$ROOT/Makefile' && grep -q '^meeting-records:' '$ROOT/Makefile' && grep -q 'Terminal Brain Freshness' '$ROOT/mac-app/scripts/freshness.zsh' && grep -q 'Terminal Brain Action Cards' '$ROOT/mac-app/scripts/action-cards.zsh' && grep -q 'Terminal Brain Daily Brief' '$ROOT/mac-app/scripts/daily-brief.zsh' && grep -q 'Terminal Brain Refresh Memory' '$ROOT/mac-app/scripts/refresh-memory.zsh' && grep -q 'Terminal Brain Meeting Records' '$ROOT/mac-app/scripts/meeting-records.zsh' && grep -q 'terminal_brain_freshness_markdown' '$ROOT/mcp-server/server.mjs' && grep -q 'terminal_brain_action_cards_markdown' '$ROOT/mcp-server/server.mjs' && grep -q 'terminal_brain_daily_brief_markdown' '$ROOT/mcp-server/server.mjs' && grep -q 'terminal_brain_refresh_memory_markdown' '$ROOT/mcp-server/server.mjs' && grep -q 'terminal_brain_meeting_records_markdown' '$ROOT/mcp-server/server.mjs'" "make freshness/action-cards/daily-brief/refresh-memory/meeting-records and MCP equivalents track staleness, refresh derived summaries, inventory local meeting records, and rank proactive next actions"
...
evidence "Manual visual certification plan" "grep -q '^visual-review-plan:' '$ROOT/Makefile' && grep -q 'Terminal Brain Visual Review Plan' '$ROOT/mac-app/scripts/visual-review-plan.zsh' && grep -q 'Use Now opens first' '$ROOT/mac-app/scripts/visual-review-plan.zsh' && grep -q 'Evidence To Capture' '$ROOT/mac-app/scripts/visual-review-plan.zsh' && grep -q 'Fail Fast Criteria' '$ROOT/mac-app/scripts/visual-review-plan.zsh' && grep -q 'Pass Evidence Template' '$ROOT/mac-app/scripts/visual-review-plan.zsh'" "non-launching checklist for the remaining visual review gap with falsifiable evidence, failure criteria, and outcome writeback"
