...
- Use `make freshness`, `make action-cards`, `make daily-brief`, or MCP `terminal_brain_freshness_markdown` / `terminal_brain_action_cards_markdown` / `terminal_brain_daily_brief_markdown` when the operator asks how to stay up to date or wants proactive guidance from their actual notes, agent histories, review queue, and repo state.
- Use `make refresh-memory` or MCP `terminal_brain_refresh_memory_markdown` when derived Codex/Claude memory is stale. Use `make meeting-records` or MCP `terminal_brain_meeting_records_markdown` for local manually exported meeting transcripts/recordings; never start recording, request microphone access, or launch meeting apps from agent tooling.
...
- Use `make playbook` or MCP `terminal_brain_playbook_markdown` when the operator needs to know which command to run for capture, Oracle reads, agent handoff, outcomes, daily cadence, or runtime checks.
- Use `make value-audit` or MCP `terminal_brain_value_audit_markdown` when you need evidence that the first-use value path is covered and a concrete list of remaining gaps.
- Use `make visual-review-plan` or MCP `terminal_brain_visual_review_plan_markdown` when you need the manual native UX certification checklist; it does not launch or foreground the app.
...
- Use `make what-now` or MCP `terminal_brain_what_now_markdown` when the operator asks what is happening now; it gives the plain answer on app focus, repo/CI state, runtime noise, what the counts mean, current blocker, and the next value command without launching the app.
...
When Terminal Brain may not be running, use `make answer`, `make start`, `make easy`, `make check-in`, `make work-block`, `make next`, `make first-minute`, `make demo`, `make playbook`, `make value-audit`, `make visual-review-plan`, `make now`, `make what-now`, `make value`, `make prove-value`, `make idea IDEA="..."`, `make review`, `make review-status ID="..." STATUS=accepted`, `make bubble-up`, `make oracle-brief`, `make status`, `make processes`, `make cleanup-plan`, `make support-bundle`, `make doctor`, `make audit`, MCP `terminal_brain_check_in_markdown`, MCP `terminal_brain_work_block_markdown`, MCP `terminal_brain_next_markdown`, MCP `terminal_brain_first_minute_markdown`, MCP `terminal_brain_demo_markdown`, MCP `terminal_brain_playbook_markdown`, MCP `terminal_brain_value_audit_markdown`, MCP `terminal_brain_visual_review_plan_markdown`, MCP `terminal_brain_now_markdown`, MCP `terminal_brain_what_now_markdown`, MCP `terminal_brain_value_now_markdown`, MCP `terminal_brain_value_proof_markdown`, MCP `terminal_brain_capture_idea`, MCP `terminal_brain_review_queue_markdown`, MCP `terminal_brain_oracle_review_status`, MCP `terminal_brain_bubble_up_markdown`, MCP `terminal_brain_oracle_brief_markdown`, MCP `terminal_brain_process_map_markdown`, MCP `terminal_brain_cleanup_plan_markdown`, MCP `terminal_brain_support_bundle_markdown`, MCP `terminal_brain_doctor_markdown`, MCP `terminal_brain_audit_markdown`, or MCP `terminal_brain_runtime_status` first. These checks do not launch or foreground the app.
