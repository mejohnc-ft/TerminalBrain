...
- Use `make what-now` or MCP `terminal_brain_what_now_markdown` when the operator asks what is happening now; it gives the plain answer on app focus, repo/CI state, runtime noise, current blocker, and the next value command without launching the app.
