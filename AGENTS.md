...
When Terminal Brain may not be running, use `make first-minute`, `make now`, `make value`, `make prove-value`, `make idea IDEA="..."`, `make review`, `make review-status ID="..." STATUS=accepted`, `make oracle-brief`, `make next`, `make status`, `make processes`, `make cleanup-plan`, `make support-bundle`, `make doctor`, `make audit`, MCP `terminal_brain_first_minute_markdown`, MCP `terminal_brain_now_markdown`, MCP `terminal_brain_value_now_markdown`, MCP `terminal_brain_value_proof_markdown`, MCP `terminal_brain_capture_idea`, MCP `terminal_brain_review_queue_markdown`, MCP `terminal_brain_oracle_review_status`, MCP `terminal_brain_oracle_brief_markdown`, MCP `terminal_brain_next_markdown`, MCP `terminal_brain_process_map_markdown`, MCP `terminal_brain_cleanup_plan_markdown`, MCP `terminal_brain_support_bundle_markdown`, MCP `terminal_brain_doctor_markdown`, MCP `terminal_brain_audit_markdown`, or MCP `terminal_brain_runtime_status` first. These checks do not launch or foreground the app.
