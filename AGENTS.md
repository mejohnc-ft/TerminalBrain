...
The handoff combines the Operator Brief, Blindspot Brief, Decision Lane, Operator Deck, Project Memory, and latest context pack. It never launches or foregrounds Terminal Brain. If using MCP, prefer `terminal_brain_handoff_markdown` as the first read, then use the Blindspot Brief, Decision Lane, Project Memory, Operator Deck, and Start Work tools for follow-up actions.
